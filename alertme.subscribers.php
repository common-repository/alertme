...
function alert_me_export_body($dest, $return=false) { 
	global $wpdb;
	$table_name = $wpdb->prefix . "alert_me"; 
...
class Alert_Me_Subscribers_List extends WP_List_Table {
	...
	public function __construct() {
		...
		parent::__construct( [
			'singular' => __( 'Singature', 'sp' ), //singular name of the listed records
			'plural'   => __( 'Singatures', 'sp' ), //plural name of the listed records
	...
	public function no_items() {
		_e( 'No subscribers currently.', 'sp' );
	}
	...
	public function column_default( $item, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'is_unsubscribed':
				return (($item[ $column_name ] == 0)? 'Active' : 'Unsubscribed' );
			case 'created_at':
	...
	function get_columns() {
		$columns = [
			...
			'id'    => __( 'ID', 'sp' ),
			'email'    => __( 'Email', 'sp' ),
			'post_id'   => __( 'Post/Page', 'sp' ),
			'is_unsubscribed' => __( 'Subscribe Status', 'sp' ),
			'created_at'	=> __( 'Calculated Date', 'sp' ),
		];
	...
	public function prepare_items() {
		...
		$per_page     = $this->get_items_per_page( 'alert_me_per_page', 10 );
...
class AlertMe_load_Subscriber_page {
	...
	public function plugin_menu() {
		...
		$hook = add_submenu_page( 'tools.php', 'Alert Me Subscribers', 'AlertMe Subscribers',
